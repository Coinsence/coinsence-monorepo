...
const KitAddresses = require('./addresses/CoinsenceKit.json');
...
class Coinsence {
  ...
  init(names) {
    ...
    return this.provider.getNetwork().then(network => {
      ...
      this.addresses['CoinsenceKit'] = this.addresses['CoinsenceKit'] || KitAddresses[network.chainId.toString()];
  ...
  get Kit() {
    return this.contractFor('CoinsenceKit');
  }
